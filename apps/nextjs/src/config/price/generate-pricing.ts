...
const getBenefits = (product: any) => {
    const marketingFeatures = product.marketing_features?.map((feature: any) => feature.name) || [];
...
const findPrices = (productId: string, options?: {
    currency?: string;
    interval?: string;
    active?: boolean
}) => {
    return pricesData.data.filter((price: any) => {
        ...
        const matchesCurrency = !options?.currency || price.currency === options.currency;
        const matchesInterval = !options?.interval || price.recurring?.interval === options.interval;
        const matchesActive = options?.active === undefined || price.active === options.active;

        return matchesProduct && matchesCurrency && matchesInterval && matchesActive;
    });
};
...
const createLocaleData = (locale: string): SubscriptionPlanTranslation[] => {
   ...
   */
    ...
    const upperCurrencyFilter = (localeCurrencyFilter[locale] || localeCurrencyFilter['en'] || 'usd').toUpperCase();
          ...
          /*
            ...
            */
            const prices = price_perProduct.filter((price: any) => {
                ...
                return price.currency.toUpperCase() === upperCurrencyFilter ||

                    (price.currency_options &&
                        Object.keys(price.currency_options).map(key => key.toUpperCase()).includes(upperCurrencyFilter));
            });
            ...
            const monthlyPrice = prices.find((price: any) => price.recurring?.interval === 'month');
            const yearlyPrice = prices.find((price: any) => price.recurring?.interval === 'year');
