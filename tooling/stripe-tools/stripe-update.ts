...
import {
  pathPrice,
  pathProduct,
} from "../../apps/nextjs/src/config/price/generate-pricing";
...
async function importData() {
  ...
  for (const product of products.data) {
    ...
    const newProductJson: Stripe.ProductCreateParams =
    ...
    if (existingProduct) {
      ...
      newProduct = await stripe.products.update(
        existingProduct.id,
        newProductJson,
      );
    } else {
    ...
    for (const price of productPrices) {
      ...
      const existingPrice = existingPrices.data.find(
        (p) =>
          p.unit_amount === price.unit_amount && p.currency === price.currency,
      );

      const removeUnitAmountFromCurrencyOptions = (
        ...
        const hasNullUnitAmountDecimal = Object.values(
          price.currency_options,
        ).some((option) => option.unit_amount_decimal === null);
      ...
      if (updatedPrice?.currency_options === null) {
        ...
        const createPriceJson: Stripe.PriceCreateParams = createPriceObject(
          updatedPrice,
          newProduct,
        );
